package io.github.promptt001.coordinatecracker.math;

public class Vector3 {

	private int x;
	private int y;
	private int z;
	
	/**
	* Vector3 constructor given x, y and z values
	*/
	public Vector3(int x, int y, int z) {
		this.x = x;
		this.y = y;
		this.z = z;
	}

	
	/**
	* Vector3 constructor given a Vector3 instance to copy
	* vector3: Vector3 instance to copy
	*/
	public Vector3(Vector3 vector3) {
		this.x = vector3.getX();
		this.y = vector3.getY();
		this.z = vector3.getZ();
	}
	
	
	/**
	* Returns x-value
	*/
	public int getX() {
		return this.x;
	}

	
	/**
	* Returns y-value
	*/
	public int getY() {
		return this.y;
	}

	
	/**
	* Returns z-value
	*/
	public int getZ() {
		return this.z;
	}
	
	
	/**
	* Returns absolute x-value
	*/
	public int getAbsX() {
		return this.x < 0 ? this.x * -1 : this.x;
	}
	
	
	/**
	* Returns absolute y-value
	*/
	public int getAbsY() {
		return this.y < 0 ? this.y * -1 : this.y;
	}
	
	
	/**
	* Returns absolute z-value
	*/
	public int getAbsZ() {
		return this.z < 0 ? this.z * -1 : this.z;
	}

	
	/**
	* Sets x to new value
	* x: new value
	*/
	public void setX(int x) {
		this.x = x;
	}

	
	/**
	* Sets y to new value
	* y: new value
	*/
	public void setY(int y) {
		this.y = y;
	}

	
	/**
	* Sets z to new value
	* z: new value
	*/
	public void setZ(int z) {
		this.z = z;
	}
	
	
	/**
	* Increments x by given value
	* value: value to increment by
	*/
	public void incrementX(int value) {
		this.x += value;
	}
	
	
	/**
	* Increments y by given value
	* value: value to increment by
	*/
	public void incrementY(int value) {
		this.y += value;
	}
	
	
	/**
	* Increments z by given value
	* value: value to increment by
	*/
	public void incrementZ(int value) {
		this.z += value;
	}
	
	
	/**
	* Subtracts another Vector3 instance from itself and returns the result as a new Vector3 instance
	*/
	public Vector3 subtract(Vector3 vec2) {
		return new Vector3(this.x - vec2.x, this.y - vec2.y, this.z - vec2.z);
	}
	
	
	/**
	* Multiplies another Vector3 instance with itself and returns the result as a new Vector3 instance
	*/
	public Vector3 multiply(Vector3 vec3) {
		return new Vector3(this.x * vec3.x, this.y * vec3.y, this.z * vec3.z);
	}
	
}
